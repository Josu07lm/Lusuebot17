...
const {
  ...
  *${mdata.subject}*
  ...
  Usa ${prefix}verify para verificarte y poder usar el bot, recuerda no pedir muchos comandos seguidos.`
				  Lxa.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			  } else if (anu.action == 'remove') {
				  num = anu.participants[0]
				  teks = `Que bueno que te fuiste joto @${num.split('@')[0]} no te queriamos 👋`
				  Lxa.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			  }
		  } catch (e) {
	  ...
	  Lxa.on('chat-update', async (mek) => {
		...
		const is = budy.slice(0).trim().split(/ +/).shift().toLowerCase()
			  mess = {
	wait: '⏳ Espera por favor, no pidas más comandos',
	success: '🎉 Con exito',
  ...
  Lxa.sendMessage(from, `Maaf limit kamu telah habis, silahkan lakukan pembelian ulang`, text,{ quoted: mek})
		found = true
								  }
						  }
		if (found === false) {
  ...
  *〘 FUN 〙*
  ...
  ┠ _Compra de límites_
  ┠⊷️ *${prefix}Transfer
  ┠ _Transferencia de dinero_
  ┠⊷️ *${prefix}CheckLimit o Check limit user
  ┠ _Averigua tu límite 
  ┠⊷️ *${prefix}Alay (Texto)*
  ┃ _Font Alay_
 ...
 */
  ...
  *OVO :* _${Ovo}_`,
  ...
  await reply(`〘  *NOTA DE PAGO*  〙\n\n‣ *Remitente* : Admin\n‣ *Recepción de la hora* : ${pushname}\n‣ *Importe de la compra* : ${body.slice(10)} \n‣ *Precio límite* : ${koinPerlimit}/limit\n‣ *Balance restante* : ${checkATMuser(sender)}\n\nProceso realizado correctamente con el número de pago \n${createSerial(15)}')
	}
	break
  ...
  *${prefix}Objetivo de etiqueta de transferencia| Importe de transferencia*`)
	if (!q.includes('|')) return  reply('No incluyas nada más')
	const tujuan = q.substring(0, q.indexOf('|') - 1)
	const jumlah = q.substring(q.lastIndexOf('|') + 1)
	if (isNaN(jumlah)) return await reply('No agregues ninguna marca  !')
	if (jumlah < 5000) return reply(`transferencia mínima Rp.100`)
  ...
  addKoinUser('593992390339@s.whatsapp.net', fee)
  await reply(`〘  *TRANSFERENCIA*  〙
	La entrega de saldos ha sido exitosa
	‣ *De* : ${sender.split("@")[0]}
	‣ *Ir * : ${tujuan}
	‣ *Importe de transferencia* : Rp.${jumlah},-
	‣ *Tarifas de administración* : Rp.${fee},-`)
	}
